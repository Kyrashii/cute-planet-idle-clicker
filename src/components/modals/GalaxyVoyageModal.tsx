...
export const GalaxyVoyageModal: React.FC<GalaxyVoyageModalProps> = React.memo(({
  ...
  return (
    ...
    >
        ...
        <div className={`absolute top-4 right-4 sm:top-6 sm:right-6 animate-pulse select-none text-2xl ${inGlitchGalaxy ? "text-cyan-400 glitch-chromatic-text" : "text-cosmic-pink"}`}>
          {inGlitchGalaxy ? "⧉" : "✨"}
        </div>
        <div className={`absolute top-8 left-6 sm:top-12 sm:left-12 select-none text-xl animate-bounce ${inGlitchGalaxy ? "text-rose-400" : "text-cosmic-accent"}`} style={{ animationDuration: "4s" }}>
          {inGlitchGalaxy ? "⚙️" : "🪐"}
        </div>
        ...
        <div className="relative z-10 space-y-4 sm:space-y-6 flex flex-col items-center text-center">
          ...
          <div className="flex flex-col items-center">
            ...
            >
              <Rocket className={`w-3.5 h-3.5 ${inGlitchGalaxy ? "text-cyan-400 animate-pulse" : "text-cosmic-pink"}`} />
              <span className={`text-[10px] font-mono font-black uppercase tracking-[0.2em] ${inGlitchGalaxy ? "text-cyan-300" : "text-[#ffcbdc]"}`}>
                {inGlitchGalaxy ? "⚡ QUANTUM_RECONSTRUCTION_READY ⚡" : "Evolutionäre Vollendung"}
              </span>
            </motion.div>
            
            <h1 className={`font-sans font-black uppercase text-2xl sm:text-4xl tracking-widest drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)] ${
            ...
            <p className="font-mono text-[10px] sm:text-xs text-cosmic-accent font-black tracking-wider uppercase opacity-90 mt-1">
              Planet Level 20 erreicht • {inGlitchGalaxy ? "REALITÄT STABILISIEREN" : "Bereit für den nächsten Kosmos"}
            </p>
          </div>
          ...
          <div className={`p-4 sm:p-6 rounded-[2rem] max-w-xl w-full shadow-[0_4px_30px_rgba(0,0,0,0.4)] backdrop-blur-sm self-center border-2 ${
            inGlitchGalaxy 
            ...
            <div className="flex justify-center mb-2.5 text-rose-300">
              <Heart className={`w-5 h-5 fill-rose-300/30 animate-pulse ${inGlitchGalaxy ? "text-cyan-400" : "text-cosmic-pink"}`} />
            </div>
            <p className={`font-sans font-semibold text-xs sm:text-sm leading-relaxed italic ${inGlitchGalaxy ? "text-rose-300 font-mono" : "text-cosmic-text"}`}>
              {inGlitchGalaxy ? (
          ...
          <div className="grid grid-cols-3 gap-2.5 max-w-lg w-full">
            ...
            <div className="p-2 w-full bg-[#181333]/90 border border-cosmic-accent/40 rounded-xl flex items-center gap-2 justify-center">
              ...
              <div className="text-left">
                <span className="block text-[8px] font-mono leading-none font-bold text-cosmic-accent-muted uppercase">{inGlitchGalaxy ? "SPLITTER & DUST" : "Währung"}</span>
          ...
          >
            ...
            <span>{inGlitchGalaxy ? "SYSTEM RE-BOOT & SPEICHERN! 🛠️" : "Jetzt Galaxiereise antreten! 🚀"}</span>
          </motion.button>
